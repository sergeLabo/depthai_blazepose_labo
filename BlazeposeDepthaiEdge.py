...
class BlazeposeDepthai:
    ...
    def __init__(self, **config):
        """
        ['build_manager_script', 'create_pipeline', 'crop', 'crop_w', 'device',
        'divide_by_255_model', 'exit', 'force_detection', 'fps', 'frame_size',
        'img_h', 'img_w', 'input_type', 'internal_fps', 'is_present', 'laconic',
        'lm_input_length', 'lm_model', 'lm_postprocess', 'lm_score_thresh',
        'nb_frames_no_body', 'nb_kps', 'nb_lm_inferences',
        'nb_lm_inferences_after_landmarks_ROI', 'nb_pd_inferences', 'next_frame',
        'pad_h', 'pad_w', 'pd_input_length', 'pd_model', 'pd_score_thresh',
        'pp_model', 'presence_threshold', 'q_manager_out', 'q_video',
        'rect_transf_scale', 'scale_nd', 'smoothing', 'stats', 'trace',
        'video_fps', 'visibility_threshold', 'xyz']
        """

        input_src = config.get('input_src')
        pd_model = config.get('pd_model')
        pd_score_thresh = config.get('pd_score_thresh')
        pp_model = config.get('pp_model')
        lm_model = config.get('lm_model')
        lm_score_thresh = config.get('lm_score_thresh')
        xyz = config.get('xyz')
        crop = config.get('crop')
        smoothing = config.get('smoothing')
        filter_window_size = config.get('filter_window_size')
        filter_velocity_scale = config.get('filter_velocity_scale')
        stats = config.get('stats')
        internal_fps = config.get('internal_fps')
        internal_frame_height = config.get('internal_frame_height')
        trace = config.get('trace')
        force_detection = config.get('force_detection')
